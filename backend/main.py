"""
main.py — FastAPI backend for Cloud Resume Screener v2
All 5 unique features + spaCy NLP + Neon PostgreSQL + AWS S3 + StaticFiles
"""
import os, sys
...
from database import get_db, init_db, JobPosting, Resume, ScreeningSession
from nlp_engine import rank_resumes, extract_all, SKILL_TAXONOMY, compute_gap_analysis
...
app = FastAPI(
    ...
    description="NLP-powered resume screening — spaCy · TF-IDF · Neon PostgreSQL · AWS S3",
    version="2.0.0",
    lifespan=lifespan,
)
...
def get_jobs(db: Session = Depends(get_db)):
    jobs = db.query(JobPosting).filter(JobPosting.is_active == True)\
...
async def upload_resumes(
    ...
    # Re-save all (re-ranked with updated IDF)
    ...
    return {
        "message": f"Processed {len(new_texts)} new resume(s). Total: {len(ranked)}",
        "total": len(ranked),
        "new": len(new_texts),
    }
...
@app.get("/api/jobs/{job_id}/resumes/{resume_id}")
def get_resume_detail(job_id: int, resume_id: int, db: Session = Depends(get_db)):
...
# ── Unique Feature 2: Skill Heatmap ───────────────────────────────────────────
@app.get("/api/jobs/{job_id}/heatmap")
def skill_heatmap(job_id: int, db: Session = Depends(get_db)):
    """
    Returns a matrix: candidates × skill categories, with counts.
    Used to render the skill coverage heatmap in the dashboard.
    """
    job = db.query(JobPosting).filter(JobPosting.id == job_id).first()
    if not job:
        raise HTTPException(404, "Job not found")
...
# ── Unique Feature 3: Export PDF / CSV ────────────────────────────────────────
...
def export_pdf(job_id: int, db: Session = Depends(get_db)):
    ...
    filename  = f"screening_{job.title.replace(' ','_')}_{datetime.now(timezone.utc).strftime('%Y%m%d')}.pdf"
    return Response(
        content=pdf_bytes,
...
def export_csv(job_id: int, db: Session = Depends(get_db)):
    ...
    filename = f"screening_{job.title.replace(' ','_')}_{datetime.now(timezone.utc).strftime('%Y%m%d')}.csv"
    return Response(
...
# ── Unique Feature 5: Batch Comparison (top N radar data) ─────────────────────
@app.get("/api/jobs/{job_id}/compare")
def compare_top_candidates(job_id: int, top: int = 3, db: Session = Depends(get_db)):
    """
    Returns score breakdown for top N candidates, ready for radar chart comparison.
    """
    resumes = db.query(Resume).filter(Resume.job_id == job_id)\
...
def analytics_overview(db: Session = Depends(get_db)):
    ...
    recent        = db.query(ScreeningSession)\
                      .order_by(ScreeningSession.created_at.desc()).limit(5).all()
    return {
...
# ── Serve React frontend via StaticFiles ───────────────────────────────────────
